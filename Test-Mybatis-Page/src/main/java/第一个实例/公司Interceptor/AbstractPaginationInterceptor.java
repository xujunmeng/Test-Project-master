...
public abstract class AbstractPaginationInterceptor implements Interceptor {
	...
	private static final Logger log = LoggerFactory.getLogger(AbstractPaginationInterceptor.class);
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		// 判断是否需要分页操作
		if (parameterObject instanceof PagingCriteria && ((PagingCriteria)parameterObject).isPaging()) {

			String originalSql = (String) metaStatementHandler.getValue("delegate.boundSql.sql");

			PagingCriteria criteria = (PagingCriteria) parameterObject;

			String pageSQL = getLimitString(originalSql, criteria.getPageIndex() * criteria.getPageSize(), criteria.getPageSize());
